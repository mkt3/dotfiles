...
** Constant
   #+begin_src emacs-lisp
     ...
     (defconst my:d:org-hugo (or (getenv "ORG_HUGO") ""))
     ...
     #+begin_src emacs-lisp
       (use-package org-agenda
         ...
         (defun my:copy-calendar ()
           "Start the calendar copy script."
           (start-process
            "copy-calendar" nil "bash"
            (expand-file-name "personal_config/calendar/copy_calendar.sh"
                              my:d:data-dir)))
...
** llm
   #+begin_src emacs-lisp
     ...
     (use-package ellama
       ...
       (require 'llm-gemini)
       (unless ellama-provider
         (setopt ellama-provider
                 (make-llm-gemini
                  :key (plist-get (nth 0 (auth-source-search :app "gemini" :max 1)) :api-key)
   ...
   #+begin_src emacs-lisp
     (use-package consult-recoll
       ...
       (defun my:update-recoll-index ()
         "Start a Recoll index update when its configuration is available."
         (let ((config-directory (getenv "RECOLL_CONFDIR")))
           (when (and config-directory (> (length config-directory) 0))
       ...
       (when (and (display-graphic-p) (getenv "RECOLL_CONFDIR"))
