...
**** ox-hugo
     #+begin_src emacs-lisp
       (use-package ox-hugo
         ...
         (defun my:org-hugo-custom-output (orig-fun &optional async subtreep visible-only)
           "Customize the output directory and file name for `org-hugo-export-to-md`
       based on ID and #+lang, placing the output inside `org-hugo-base-dir/content/posts/`."
           (save-excursion
             (goto-char (point-min))
             (let* ((base-dir (expand-file-name "content/posts/" org-hugo-base-dir))
                    (id (when (re-search-forward "^:ID: +\\([a-zA-Z0-9-]+\\)" nil t)
                          (match-string 1)))
                    (lang (or (when (re-search-forward "^#\\+lang: +\\([a-zA-Z0-9-]+\\)" nil t)
                                (match-string 1))
                              "en"))
                    (custom-dir (expand-file-name id base-dir))
                    (custom-filename (format "index.%s.md" lang))
                    (outfile (expand-file-name custom-filename custom-dir)))
               (unless (file-exists-p custom-dir)
                 (make-directory custom-dir t))
               (prog1
                   (org-export-to-file 'hugo outfile async subtreep visible-only)
                 (message "Exported Hugo markdown to: %s" outfile)))))

         (advice-add 'org-hugo-export-to-md :around #'my:org-hugo-custom-output)
