...
** Whitespace
   #+begin_src emacs-lisp
     (use-package whitespace
       ...
       :hook
       (org-src-mode-hook . whitespace-turn-on))
   #+end_src
* UI
** Clipboard
   #+begin_src emacs-lisp
   ...
   #+begin_src emacs-lisp
     (use-package go-translate
       ...
       (when (file-directory-p my:d:password-store)
         ;; (setq engine-list (append '((gts-deepl-engine :auth-key (plist-get (nth 0 (auth-source-search :app "deepl" :max 1)) :api-key) :pro nil)) engine-list))
         )
